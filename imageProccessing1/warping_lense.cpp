...
vector<Point2f> findImageCorners(Mat image, Size boardSize)
{
	...
	/*
	 *2번째 인자는 1번째 인자와 사이즈와 뎁스가 같은 이미지로 출력될 인자.
	 *이때 gray의 사이즈가 결정됨
	 */
	...
	*/
	...
	if (found)
	{
		cornerSubPix(gray, imgPoints, Size(5, 5), Size(-1, -1),
			/*TermCriteria() = (만족 조건, 최대 반복횟수, 정확도).
			 *즉, 아래 코드는 최대 횟수(30회)만큼 반복하고, 정확도(0.1)보다 낮아진 후에 종료됨.
		...
		* 코너 위치를 서브 픽셀수준의 정확도를 얻기 위해 다시 한번 전처리 공정을 거침
		*
		* 첫 인자는 입력 이미지
...
int main()
{
	...
	// 8너비, 7높이 bord 선언, 검은색/흰색의 칸 수 X, 검흰검흰으로 이루어진 사각형의 센터값이 기준
	...
	*
	* 첫번째 인자는 calcObjectCorners를 통해 얻어진 비교군 좌표들의 집합
	* (체커보드를 바꾸지 않았다면 모두 동일)
	* 두번째 인자는 샘플 이미지들의 집합
	* 네번째 인자는 함수를 통해 얻어낸 카메라 내부 파라미터 값을 반환해줄 변수
	* 다섯번째 인자는 왜곡 계수 "행렬"
	* 6번째, 7번째 인자는 3*1 행렬로 이루어진 백터
	* 각 행렬의 3가지 데이터가 의미하는 바는 yow pitch roll 방향의 회전/이동 백터
	*/
