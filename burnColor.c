...
int main(int argc, char** argv)
{
	...
	for(int j = 0 ; j <256 ; j++)
	{
    	...
    	palrgb[j].rgbGreen= j; 
     	palrgb[j].rgbRed= j; 
       	palrgb[j].rgbReserved= j; 
	}

	for(i = 0 ; i < height * width * 3; i += 3) 
	{ 
		outimg[i/3] = 0.114*inimg[i] + 0.587*inimg[i+1] + 0.299*inimg[i+3];
	}

	bits = 8;
	imagesize = (height * width * bits)/8;
	file_size = imagesize+ sizeof(palrgb)+imagesize;
	ncolors = 0;
