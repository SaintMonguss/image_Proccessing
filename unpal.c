...
int main(int argc, char** argv) {
	...
	inimg=(BYTE*)malloc(sizeof(BYTE)*imagesize);  //원본 이미지를 저장할 메모리 할당
	outimg=(BYTE*)malloc(sizeof(BYTE)*width*height*3); //
	fread(inimg, sizeof(BYTE), imagesize, fp); 
	
	fclose(fp);

	int movePos = 0; // 변환할 이미지의 픽셀 특정을 위한 변수
	...
	for(i=0; i<height*width/8; i++)
		for(int j = 7; j >= 0; j--)
		{
			int num = inimg[i];
			int val = num >> j & 1;
			outimg[movePos++]= palrgb[val].rgbBlue;
			outimg[movePos++]= palrgb[val].rgbGreen;
			outimg[movePos++]= palrgb[val].rgbRed;
		}
